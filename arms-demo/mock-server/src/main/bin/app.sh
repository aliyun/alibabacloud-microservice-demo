...
echo "current id is ${MOCK_SERVER_ID}"

APP_NAME="sanmu-aiops-demo-server"
ARMS_NAME="$APP_NAME"
if [ "$CURRENT_ENV" = "dev" ]; then
  ARMS_NAME="dev_${ARMS_NAME}";
fi
...
JAVA_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=9999 -Xms256m -Xmx1024m -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75 -XX:MaxDirectMemorySize=256m"\
" -XX:+UseCMSInitiatingOccupancyOnly -XX:SurvivorRatio=8 -XX:+ExplicitGCInvokesConcurrent -XX:MetaspaceSize=128m"\
" -XX:MaxMetaspaceSize=256m -XX:-OmitStackTraceInFastThrow -XX:+HeapDumpOnOutOfMemoryError -XX:+EnableJFR"\
...
if [ "$SERVICE_NAME" = "insights-memory-case" ]; then

  JAVA_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=9999 -Xms256m -Xmx256m -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75 -XX:MaxDirectMemorySize=56m"\
" -XX:+UseCMSInitiatingOccupancyOnly -XX:SurvivorRatio=8 -XX:+ExplicitGCInvokesConcurrent"\
" -XX:-OmitStackTraceInFastThrow -XX:+HeapDumpOnOutOfMemoryError -XX:+EnableJFR"\
" -XX:HeapDumpPath=${BASEDIR}/logs -Djava.io.tmpdir=${BASEDIR}/logs -XX:+PrintGCDetails -XX:+PrintGCDateStamps"\
" -Xloggc:${BASEDIR}/logs/gc-%t.log -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=5 -XX:GCLogFileSize=10m"\

fi

if [ "$SERVICE_NAME" = "insights-http-server" ]; then
