...
func greet(ctx context.Context, name, httpType string) (*GreetRespData, error) {
	if httpType == http.MethodGet {
		return getGreet(ctx, name, "go-kratos-demo-b")
	}
	return postGreet(ctx, name, "go-kratos-demo-b")
}
...
func getGreet(ctx context.Context, name, disService string) (*GreetRespData, error) {
	...
	respData := &GreetResp{}
	var httpRespData GreetRespData
	defer resp.Body.Close()
	if err := json.NewDecoder(resp.Body).Decode(&httpRespData); err != nil {
		return nil, err
	}
	fmt.Println("[getGreet] httpRespData ", httpRespData)
	...
	respData.Data.Greeting = httpRespData.Greeting
	respData.Data.CallChain = fmt.Sprintf("A:%s:%s", tag, ip) + " -(http)- " + httpRespData.CallChain
